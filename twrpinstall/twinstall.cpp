...
// An updater rebuilding dynamic partitions has to unmap them, which fails while
// anything is still executing from the mount
struct HolderCtx {
	std::set<pid_t>* pids;
...
static void Kill_Holders(const std::string& mount_point) {
	...
	// Stop them by name first, or init just restarts them as soon as they die
	std::vector<std::string> services;
	...
	// The mount stays busy until they are actually reaped
	for (int i = 0; i < 50; i++) {
		bool alive = false;
		for (std::set<pid_t>::iterator it = holders.begin(); it != holders.end(); ++it) {
			if (kill(*it, 0) == 0) {
